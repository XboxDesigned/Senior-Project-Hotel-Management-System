...
$query = "SELECT confirmation_num, guest_id, room_num, check_out_date FROM reservations WHERE check_out_date = CURDATE() AND status = 'checked-out'";
$statement = $db->prepare($query);
$statement->execute();
...
$query = "SELECT confirmation_num, guest_id, room_num, check_in_date FROM reservations WHERE check_in_date = CURDATE() AND status = 'no-show'";
$statement = $db->prepare($query);
$statement->execute();
...
// Fetch Daily Revenue
$query = "SELECT SUM(balance) AS daily_revenue FROM reservations WHERE check_out_date = CURDATE() AND status = 'checked-out'";
$statement = $db->prepare($query);
$statement->execute();
...
<body>
    ...
    <table>
        ...
        <?php foreach ($checkouts as $row) { ?>
            <tr><td><?= htmlspecialchars($row["confirmation_num"]) ?></td><td><?= htmlspecialchars($row["guest_id"]) ?></td><td><?= htmlspecialchars($row["room_num"]) ?></td><td><?= htmlspecialchars($row["check_out_date"]) ?></td></tr>
        <?php } ?>
    </table>
    ...
    <table>
        ...
        <?php foreach ($noshow as $row) { ?>
            <tr><td><?= htmlspecialchars($row["confirmation_num"]) ?></td><td><?= htmlspecialchars($row["guest_id"]) ?></td><td><?= htmlspecialchars($row["room_num"]) ?></td><td><?= htmlspecialchars($row["check_in_date"]) ?></td></tr>
        <?php } ?>
    </table>
